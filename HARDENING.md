...
**Machine:** hormiga (Ubuntu 22.04.5 LTS, kernel 5.15.0-171-generic)
**Role:** Headless workstation — no GUI, SSH access only
**Date:** 2026-03-14
...
**What:** UFW is enabled with a default-deny inbound policy. Only SSH (port 22/tcp) is allowed inbound. All outbound traffic is permitted.

**Why:** Prevents any service accidentally exposed on the network from being reachable. Only SSH is required for remote administration.
...
**What:** fail2ban is enabled with the `sshd` jail active, using the systemd backend (this is a minimal install without rsyslog).
...
**What:** The system was installed as a minimal Ubuntu Server with no desktop environment, window manager, or X11 session. No web servers, database servers, or other network-facing services are installed.
...
|------|---------|---------|---------|
| 22/tcp | sshd | 0.0.0.0 + [::] | Remote administration |
...
No other ports are open. No services are exposed to the network besides SSH.

### 1.4 Automatic security updates

**What:** `unattended-upgrades` is installed and enabled. The system automatically downloads and installs security updates daily.
...
**What:** AppArmor is loaded and enforcing with 9 profiles active (Ubuntu default). Covers NetworkManager DHCP helpers and other system components.
...
**What:** The operator account uses standard `sudo` with password authentication for all privileged operations. Direct root login is not used. The root account has no password set (Ubuntu default) — `su` to root is not possible.
...
**Note:** During automated setup, a temporary NOPASSWD sudo entry is created to allow unattended script execution. This is removed as the final step of the setup process and is not present in normal operation.

### 1.7 No console autologin (default state)

**What:** The physical console (TTY) requires standard username/password authentication. There is no automatic login.

**Why:** Autologin on the physical console would allow anyone with physical access to obtain a logged-in shell without credentials. While the machine is on a private network, physical access controls are not a substitute for authentication.

**Optional override:** A toggle script (`toggle-autologin.sh`) is provided for situations where the operator needs the machine to boot into a monitoring dashboard (e.g., for a display showing system status). Autologin can be enabled temporarily and disabled again when no longer needed. See `toggle-autologin.sh --help`.
...
**What:** Secure Boot was disabled in BIOS to allow the NVIDIA proprietary kernel module (DKMS) to load.

**Why it was necessary:** The NVIDIA driver installs a kernel module via DKMS. With Secure Boot enabled, the module must be signed with a Machine Owner Key (MOK), which requires interactive enrolment at the UEFI console on each kernel update. On a headless workstation managed remotely, this is impractical — a failed MOK enrolment bricks GPU support until someone is physically present.
...
- The machine is on a private network
- Physical access is controlled
- The threat model prioritises remote attacks over physical tampering

### 2.2 SSH allows password authentication

**What:** Password authentication over SSH is not explicitly disabled. The default Ubuntu 22.04 sshd configuration allows both password and public key authentication.

**Why not changed:** This is a conscious decision for initial setup flexibility. If key-only auth is enforced and the key is lost, the only recovery path is physical console access.

**Recommendation:** Once SSH keys are deployed and verified, consider hardening to key-only:
```bash
# /etc/ssh/sshd_config
...
**Why not:** The machine is not a router — ICMP redirect sending is low-risk on an endpoint. Martian logging is useful for network debugging but noisy in normal operation. These can be applied via `/etc/sysctl.d/99-hardening.conf` if the threat model tightens.

### 3.4 Disk encryption (LUKS)

**Why not:** Must be configured at install time (or requires significant repartitioning). On a headless machine, LUKS requires entering a passphrase at boot via console (or configuring network-based unlock via Clevis/Tang), which adds operational complexity. If the machine stores sensitive data, this should be revisited.

### 3.5 SSH key-only authentication (see 2.2)

Recommended as a follow-up step once keys are deployed and tested.

### 3.6 CIS Benchmark / full hardening profile

**Why not:** The CIS Ubuntu 22.04 benchmark includes ~250 checks covering everything from filesystem mount options to cron permissions. Many controls target multi-user servers or GUI workstations and are not applicable to a single-user headless machine. Individual controls from CIS can be adopted as needed.

### 3.7 Intrusion detection (AIDE, rkhunter, chkrootkit)

**Why not:** File integrity monitoring (AIDE) and rootkit scanners are valuable on internet-facing servers. On an internal workstation with SSH as the only entry point, protected by fail2ban and UFW, the risk is lower. Can be added if the machine's exposure changes.

### 3.8 Idle session timeout

**Why not:** SSH sessions could be configured with `ClientAliveInterval` / `ClientAliveCountMax` in sshd_config, but this can disrupt long-running interactive work (compilation, training jobs, etc.). The operator is the sole user of the machine, so unattended session risk is low.

### 3.9 Login banner / legal warning
...
- Target hosts configured in `security/hosts.txt`
...
**Setup:**
```bash
# Run the local audit interactively (first run creates a baseline)
sudo ./security/audit-local.sh

# Add lab machine IPs to hosts.txt, then scan remotely
nano ./security/hosts.txt
sudo ./security/audit-remote.sh

# Install cron jobs for automated monitoring
sudo ./security/setup-cron.sh
```
...
- The lab grows beyond 4–5 machines
...
**Agent resource impact:** The Wazuh agent is lightweight in steady state (~35–50 MB RAM, negligible CPU). It spikes briefly during periodic integrity checks and vulnerability scans but does not impact workload performance in normal operation.
...
| AppArmor | Enforcing | 9 profiles (Ubuntu defaults) |
| Sudo with password | Enforced | All admin via sudo, no NOPASSWD |
| Root login | Disabled | No root password, no root SSH key |
| Console autologin | Disabled | Password required at physical console |
| Secure Boot | Disabled | Required for NVIDIA DKMS (see 2.1) |
| SSH password auth | Allowed | Recommend key-only after setup (see 2.2) |
